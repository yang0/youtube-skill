...
4. 路由到子 skill
   ...
   │   └─ 搜索后必须：python E:\projectHome\youtube-vault\backend\dedup_import.py --file <results.json>
   │      自动去重入库，避免下次搜索返回相同视频
...
## YouTube Vault 集成（搜索去重）

**每次搜索完成后，必须自动去重入库：**

```bash
python E:\projectHome\youtube-vault\backend\dedup_import.py \
  --file "F:\youtube_data\ai-money-videos\search_xxx\results.json" \
  --output "F:\youtube_data\ai-money-videos\search_xxx\results_new.json"
```

**效果**：
- 新视频自动入库，旧视频自动跳过
- 输出 `results_new.json` 仅包含新视频（可直接用于展示）
- 下次搜索时同一视频不会再出现在"新结果"中

**Vault UI 查看**：
```
http://127.0.0.1:8766
```
（需先启动 `python E:\projectHome\youtube-vault\backend\server.py`）
