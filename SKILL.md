...
│
├─ 要搜索视频 / 提取评论 / 下载头像 / 深挖内容？
│   ├─ ⛔ 先做意图校准（不可跳过）
│   └─ 校准完成后 → 路由到对应子 skill
│       ├─ youtube-search（搜索/评论/头像）
│       └─ use-notebooklm（深挖/教程/播客）
│
├─ 要完整链路：搜索 → 分析 → 沉淀？
│   └─ 协调两个子 skill：
...
0. 意图校准（必须）
   "你要的是实操赚钱指南、创始人故事、还是工具教程？"
   → 用户确认后映射为具体查询词

1. youtube-search
   bun scripts/search_videos.js --queries "确认后的查询词" --output-dir ./data
   → 拿到视频列表，用户挑选
...
0. 意图校准（必须）
   "你说的田野调查，更像哪种？(a) 跟哈扎族打猎的沉浸式Vlog (b) PBS/NatGeo纪录片 (c) MIT学术讲座"
   → 用户选 a → 放弃 "anthropology"，改用 "living with tribes vlog"

1. youtube-search
   bun scripts/search_videos.js --queries "living with tribes vlog, tribe visit documentary"
   → 列出视频 URL + 频道 + 观看量
```
